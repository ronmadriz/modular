<?php
/*
Template Name: NEW Solutions Fluid
...
$alternate_title = get_field('alternate_title');
// BANNER
...
echo '<h1>'.(!empty($alternate_title)?$alternate_title:get_the_title()).'</h1>'.PHP_EOL;
echo '</div>'.PHP_EOL;
echo '</div>'.PHP_EOL;
echo '</div>'.PHP_EOL;
...
$fluid_gallery = get_field('fluid_gallery');
if ($fluid_gallery['fluid_gallery']) {
	while (have_rows('fluid_gallery')):the_row();
	...
	if ($gallery_images) {
		...
		$gal_counter = 0;
		foreach ($gallery_images as $gallery_pic):
...
if (have_rows('quotes')) {
	...
	while (have_rows('quotes')) {
		the_row();
		$testimonials = get_sub_field('fluid_testimonial');
		if ($testimonials) {
...
$fluid_image_callout = get_field('fluid_image_callout');
if (have_rows('fluid_image_callout')) {
	echo '<section id="image_callout">'.PHP_EOL;
	echo '<div class="container-fluid">'.PHP_EOL;
	while (have_rows('fluid_image_callout')) {
		the_row();
		...
		if (have_rows('fluid_callout')) {
			while (have_rows('fluid_callout')) {
				the_row();
				echo '<div class="row justify-content-center item">'.PHP_EOL;
				$callout_image   = get_sub_field('fluid_image');
				$callout_title   = get_sub_field('fluid_title');
				$callout_content = get_sub_field('fluid_content');
				$callout_link    = get_sub_field('fluid_link');
				if ($callout_link) {
...
$fluid_icons = get_field('fluid_icons');
if (have_rows('fluid_icons')) {
	echo '<section id="icons">'.PHP_EOL;
	echo '<div class="container-fluid">'.PHP_EOL;
	while (have_rows('fluid_icons')) {
		the_row();
		if (have_rows('fluid_icon_group')) {
			while (have_rows('fluid_icon_group')) {
				the_row();
				...
				if (have_rows('icon')) {
					while (have_rows('icon')) {
						the_row();
						$fl_image     = get_sub_field('fluid_image');
						$fl_content   = get_sub_field('fluid_content');
						$fl_title     = get_sub_field('fluid_icon_title');
						$fl_icon_link = get_sub_field('fluid_link_to_page');
						$fl_link      = get_sub_field('fluid_link');
...
// TABLES
if (have_rows('fluid_measurement_tables')) {
	echo '<section id="measurement_tables">'.PHP_EOL;
	while (have_rows('fluid_measurement_tables')) {
		the_row();
		$m_title  = get_sub_field('fluid_title');
		$m_image  = get_sub_field('fluid_image');
		$m_tables = get_sub_field('fluid_tables');
		echo '<div class="container-fluid">'.PHP_EOL;
		...
		if (have_rows('fluid_tables')) {
			while (have_rows('fluid_tables')) {
				the_row();
				$m_table = get_sub_field('fluid_table');
				if (!empty($m_table)) {
...
if (have_rows('fluid_literature')) {
	...
	while (have_rows('download_literature')) {
		the_row();
		$lit_file  = get_sub_field('fluid_file');
		$lit_img   = get_sub_field('fluid_Image');
		$lit_title = get_sub_field('fluid_title');
...
$fluid_solutions_group = get_field('fluid_solutions_group');
if ($fluid_solutions_group['fluid_solutions']) {
	while (have_rows('fluid_solutions_group', $post_id)):the_row();
	$solution_title = get_sub_field('fluid_title');
	echo '<section id="inline_solutions">'.PHP_EOL;
	echo '<div class="container-fluid">'.PHP_EOL;
	echo '<div class="row">'.PHP_EOL;
	echo (!empty($solution_title)?'<div class="section_title col-12"><h1>'.$solution_title.'</h1></div>'.PHP_EOL:'');
	echo '</div>'.PHP_EOL;
	$solutions_repeater = get_sub_field('fluid_solutions');
	if ($solutions_repeater) {
		echo '<div class="row">'.PHP_EOL;
		while (have_rows('fluid_solutions')) {
			the_row();
			$solutions_item = get_sub_field('solutions_item');
			if ($solutions_item):
...
$case_studies = get_field('fluid_case_studies');
if (have_rows('fluid_case_studies')) {
	echo '<section id="case_study">'.PHP_EOL;
	echo '<div class="container-fluid">'.PHP_EOL;
	while (have_rows('fluid_case_studies')):the_row();
	$fluid_study_title = get_sub_field('fluid_study_title');
	$studies           = get_sub_field('fluid_studies');
	$studies_count     = count($studies);
	echo (!empty($fluid_study_title)?'<div class="row"><div class="section_title col-12"><h1>'.$fluid_study_title.'</h1></div></div>'.PHP_EOL:'');
	if ($studies && $studies_count <= 3) {
		while (have_rows('fluid_studies')) {
			the_row();
			$study_item = get_sub_field('fluid_case');
			if ($study_item):
			$study = $study_item;
...
// VIDEOS
$videos      = get_field('fluid_videos');
$video_count = count(get_field('fluid_videos'));
if ($videos && $video_count == 1):
echo '<section id="videos">'.PHP_EOL;
echo '<div class="container-fluid">'.PHP_EOL;
while (have_rows('fluid_videos')):the_row();
$v_title = get_sub_field('fluid_title');
$v_embed = get_sub_field('fluid_v_embed');
$v_desc  = get_sub_field('fluid_description');
echo '<div class="row">'.PHP_EOL;
...
$osha = get_field('fluid_osha');
if ($osha['content']) {
	echo '<section id="fluid_osha">'.PHP_EOL;
	echo '<div class="container-fluid">'.PHP_EOL;
	while (have_rows('osha', $post_id)) {
		the_row();
		$osha_title   = get_sub_field('fluid_title');
		$osha_img     = get_sub_field('fluid_image');
		$osha_content = get_sub_field('fluid_content');
...
$contact_form_code = get_field('fluid_contact_form');
