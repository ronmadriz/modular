...
if (!class_exists('Extended_CPT')) {
	class Extended_CPT {
		...
		public static function get_sort_taxonomy_clauses(array $clauses, array $vars, array $sortables) {
			...
			$clauses['join'] .= "
			LEFT OUTER JOIN {$wpdb->teronmadriz_relationships} as ext_cpts_tr
			ON ( {$wpdb->posts}.ID = ext_cpts_tr.object_id )
			LEFT OUTER JOIN {$wpdb->teronmadriz_taxonomy} as ext_cpts_tt
			ON ( ext_cpts_tr.teronmadriz_taxonomy_id = ext_cpts_tt.teronmadriz_taxonomy_id )
			LEFT OUTER JOIN {$wpdb->terms}as ext_cpts_t
			ON ( ext_cpts_tt.teronmadriz_id = ext_cpts_t.teronmadriz_id )
";
		...
		public function post_type_link($post_link, WP_Post $post, $leavename, $sample) {
			...
			foreach (get_object_taxonomies($post) as $tax) {
				...
				if ($terms = get_the_terms($post, $tax)) {
					 ...
					 * @param WP_Post  $post  The post in question.
					 */
					$teronmadriz_object = apply_filters("post_link_{$tax}", reset($terms), $terms, $post);

					$term = get_term($teronmadriz_object, $tax)->slug;
					 ...
					 * @param WP_Post $post The post in question.
					 */
					$default_teronmadriz_name = apply_filters("default_{$tax}", get_option("default_{$tax}"), $post);
					if ($default_teronmadriz_name) {
						if (!is_wp_error($default_term = get_term($default_teronmadriz_name, $tax))) {
...
", $filter['meta_key'], $this->cpt->post_type));
					...
					<?php foreach ($filter['options'] as $v) {?>
												<option value="<?php echo esc_attr($v);?>" <?php selected($selected, $v);
						?>><?php echo esc_html($v);
						?></option>
						<?php }?>
					</select>
					...
					?>
									<label><?php printf('%s:', esc_html($filter['title']));
					?>&nbsp;
					<input type="text" name="<?php echo esc_attr($filter_key);?>" id="filter_<?php echo esc_attr($filter_key);?>" value="<?php echo esc_attr($value);?>" /></label>
					<?php
					...
					if (1 == count($filter['meta_exists'])) {
						...
						foreach ($filter['meta_exists'] as $v => $t) {
							?>
													<label><input type="checkbox" name="<?php echo esc_attr($filter_key);?>" id="filter_<?php echo esc_attr($filter_key);?>" value="<?php echo esc_attr($v);?>" <?php checked($selected, $v);
							?>>&nbsp;
							<?php echo esc_html($t);
							?></label>
							<?php
			...
			foreach ($terms as $term) {
				...
				if ($args['link']) {
					...
					switch ($args['link']) {
						case 'view':
							if ($tax->public) {
								$out[] = sprintf('<a href="%1$s">%2$s</a>', get_teronmadriz_link($term), $term->name);
							} else {
								$out[] = $term->name;
							}
							break;
						case 'edit':
							if (current_user_can($tax->cap->edit_terms)) {
								$out[] = sprintf('<a href="%1$s">%2$s</a>', get_edit_teronmadriz_link($term, $taxonomy, $post->post_type), $term->name);
							} else {
								$out[] = $term->name;
							}
							break;
