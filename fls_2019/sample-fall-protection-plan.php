...
if (have_rows('tabbed_content')) {
	...
	while (have_rows('tabbed_content')) {
		...
		echo (!empty($fp_icon)?'<div class="fp_icon col-2"><img src="'.$fp_icon_src.'" alt="'.$fp_icon_alt.'" class="img-fluid"></div>'.PHP_EOL:'');
		echo (!empty($fp_title)?'<div class="fp_content_area col-10"><span class="fp_title d-block"><h2>6.1.'.$fp_counter++ .' '.$fp_title.'</h2></span>'.PHP_EOL:'');
