...
if ($industry_gallery) {
	...
	echo '<a class="carousel-control left pt-3" href="#flsCarousel" data-slide="prev"><i class="im im-angle-left"></i></a>'.PHP_EOL;
	echo '<a class="carousel-control right pt-3" href="#flsCarousel" data-slide="next"><i class="im im-angle-right"></i></a>'.PHP_EOL;
	echo '</div>'.PHP_EOL;
	echo '</div>'.PHP_EOL;
	...
	echo '<span id="thumb_left" class="th_nav d-flex align-self-start align-items-center justify-content-center"><i class="im im-angle-left"></i></span>'.PHP_EOL;
	...
	endforeach;
	echo '</div></div>';
	echo '<span id="thumb_right" class="th_nav d-flex align-self-end align-items-center justify-content-center"><i class="im im-angle-right"></i></span>'.PHP_EOL;
	echo '</div>'.PHP_EOL;
	endif;
	echo '</div>'.PHP_EOL;
	echo '</div>'.PHP_EOL;
...
echo '<section id="cta_speak"><div class="container-fluid"><div class="row"><div class="col-12"><h2 class="text-center text-uppercase"><a href="#contactFormCTA">Speak with a fall protection specialist <i class="im im-angle-right-circle"></i></a></h2></div></div></div></section>'.PHP_EOL;
...
if ($videos && $video_count == 1) {
	...
	while (have_rows('videos')) {
		...
		echo '<span class="section_title"><h2>'.$v_title.'</h2></span>'.PHP_EOL;
		echo '<span class="video">'.$v_embed.'</span>'.PHP_EOL;
		echo (!empty($v_desc)?'<span class="content">'.$v_desc.'</span>'.PHP_EOL:'');
		echo '</div>'.PHP_EOL;
		wp_reset_query();
	}
	echo '</div>'.PHP_EOL;
 ...
 */
echo '<section id="cta_blue"><h2>Tell Us About Your Fall Hazard</h2></div></section>'.PHP_EOL;
...
if (have_rows('contact_form_cta')) {
	while (have_rows('contact_form_cta')) {
		...
		echo (!empty($cfcta_img)?'<div id="cfCTA_Img" class="d-flex d-md-none justify-content-center align-items-md-end col-12 col-md-6"><img src="'.$cfcta_img['url'].'" alt="" class="img-fluid"></div>':'').PHP_EOL;
		echo (!empty($cfcta_employee_name) || !empty($cfcta_employee_role)?'<div class="col-12 col-md-6 text-right align-self-end" id="cfCTA_employeeInfo"><h3>'.$cfcta_employee_name.' <small>'.$cfcta_employee_role.'</small></h3></div>':'').PHP_EOL;
	}
}
echo '</div>'.PHP_EOL;
echo '</div>'.PHP_EOL;
