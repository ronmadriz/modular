...
if (have_rows('contact_form_cta')) {
	while (have_rows('contact_form_cta')) {
		...
		echo (!empty($cfcta_employee_name) || !empty($cfcta_employee_role)?'<div class="col-12 col-md-6" id="cfCTA_employeeInfo"><h3>'.$cfcta_employee_name.' <small>'.$cfcta_employee_role.'</small></h3></div>':'').PHP_EOL;
	}
}
echo '</div>'.PHP_EOL;
echo '</div>'.PHP_EOL;
